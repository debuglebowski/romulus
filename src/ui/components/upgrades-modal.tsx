...
export function UpgradesModal({
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className='sm:max-w-2xl bg-zinc-950 border-zinc-800' showCloseButton={false}>
				<DialogHeader>
				...
				<div className='flex gap-2 border-b border-zinc-800 pb-2'>
					<button
						...
						className={`px-3 py-1.5 text-xs font-medium rounded ${
							activeTab === 'my' ? 'bg-zinc-900 text-white' : 'text-zinc-400 hover:text-white'
						}`}
					>
						My Upgrades
					</button>
					<button
						...
						className={`px-3 py-1.5 text-xs font-medium rounded flex items-center gap-1.5 ${
							activeTab === 'enemy' ? 'bg-zinc-900 text-white' : 'text-zinc-400 hover:text-white'
						}`}
					>
						<IconEye size={12} />
						Enemy Intel
								...
								return (
									<div key={enemyData.enemyPlayerId} className='space-y-2'>
										...
										<div className='flex items-center gap-2'>
											<div className='w-3 h-3 rounded' style={{ backgroundColor: enemy?.color ?? '#666' }} />
										...
										<div className='grid grid-cols-2 gap-2 pl-5'>
											{enemyData.upgrades.map((known) => {
												...
												return (
													<div key={known.upgradeId} className='border border-zinc-800 bg-zinc-900/50 rounded p-2'>
														<div className='flex items-start justify-between gap-2'>
															<div className='flex-1'>
																<p className='text-xs font-medium text-zinc-100'>{upgrade.name}</p>
...
function UpgradeNode({ upgrade, status, canAfford, missingRequirement, isPurchasing, onPurchase }: UpgradeNodeProps) {
	...
	return (
		<div
			className={`relative flex-1 min-w-[140px] max-w-[200px] border rounded p-2 ${
				isPurchased
					? 'border-green-700 bg-green-900/40'
					: isLocked
						? 'border-zinc-800 bg-zinc-900 opacity-60'
						: canAfford
							? 'border-primary bg-zinc-900 hover:border-primary/80'
		...
		>
			...
			<div className='absolute -top-1.5 -right-1.5'>
				{isPurchased && (
					<div className='rounded bg-green-600 p-0.5'>
						<IconCheck size={10} className='text-white' />
					</div>
				)}
				{isLocked && (
					<div className='rounded bg-zinc-800 p-0.5'>
			...
			<div className='space-y-1'>
				...
				{/* Requirement warning */}
				{isLocked && missingRequirement && <p className='text-[10px] text-primary/80 leading-tight'>{missingRequirement}</p>}
					...
					<div className='pt-1'>
						{isAvailable ? (
							<button
								...
								className={`w-full px-2 py-1 text-[10px] font-medium rounded flex items-center justify-center gap-1 ${
									canAfford ? 'bg-primary text-white hover:bg-primary/90' : 'bg-zinc-800 text-zinc-400 cursor-not-allowed'
								}`}
							>
								{isPurchasing ? (
									<span>...</span>
