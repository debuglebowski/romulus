...
export function AlliancePanel({ gameId, isCapitalMoving = false, open, onOpenChange }: AlliancePanelProps) {
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className='sm:max-w-lg bg-zinc-950 border-zinc-800 z-100' showCloseButton={false}>
				<DialogHeader>
				...
				<div className='flex gap-2 border-b border-zinc-800 pb-2'>
					<button
						...
						className={`px-3 py-1.5 text-xs font-medium rounded flex items-center gap-1.5 ${
							activeTab === 'allies' ? 'bg-zinc-900 text-white' : 'text-zinc-400 hover:text-white'
						}`}
					>
						<IconUsersGroup size={12} />
						Allies
						{activeAlliances.length > 0 && (
							<span className='bg-blue-600 text-white text-[10px] px-1.5 py-0.5 rounded tabular-nums'>{activeAlliances.length}</span>
						)}
					</button>
					<button
						onClick={() => setActiveTab('pending')}
						className={`px-3 py-1.5 text-xs font-medium rounded flex items-center gap-1.5 ${
							activeTab === 'pending' ? 'bg-zinc-900 text-white' : 'text-zinc-400 hover:text-white'
						}`}
					>
						Pending
						{pendingCount > 0 && (
							<span className='bg-primary text-white text-[10px] px-1.5 py-0.5 rounded tabular-nums'>{pendingCount}</span>
						)}
					</button>
					<button
						onClick={() => setActiveTab('invite')}
						className={`px-3 py-1.5 text-xs font-medium rounded flex items-center gap-1.5 ${
							activeTab === 'invite' ? 'bg-zinc-900 text-white' : 'text-zinc-400 hover:text-white'
						}`}
					>
				...
				{/* Error message */}
				{error && (
					<div className='px-3 py-2 bg-red-900/30 border border-red-800/50 rounded'>
					...
					<div className='space-y-3 max-h-80 overflow-y-auto'>
						{activeAlliances.length === 0 ? (
							...
							activeAlliances.map((alliance) => (
								<div key={alliance._id} className='border border-zinc-800 bg-zinc-900/50 rounded p-3 space-y-3'>
									{/* Ally header */}
									<div className='flex items-center justify-between'>
										<div className='flex items-center gap-2'>
											<div
												className='w-4 h-4 rounded border-2 border-zinc-700'
										...
										<button
											...
											className='px-2 py-1 text-[10px] font-medium rounded bg-red-900/50 text-red-400 hover:bg-red-900'
										>
									...
									<div className='space-y-1.5'>
										<p className='text-[10px] text-zinc-400'>I share with them:</p>
										<div className='grid grid-cols-5 gap-1'>
											{SHARING_TYPES.map((type) => (
												<div
													key={type}
													className='flex flex-col items-center gap-1 p-1.5 rounded bg-zinc-950/50'
													title={SHARING_LABELS[type].description}
												>
													<div className={alliance.mySharing[type] ? 'text-blue-400' : 'text-zinc-600'}>
									...
									<div className='space-y-1.5'>
										<p className='text-[10px] text-zinc-400'>They share with me:</p>
										<div className='grid grid-cols-5 gap-1'>
											{SHARING_TYPES.map((type) => (
												<div
													key={type}
													...
													title={SHARING_LABELS[type].description}
												>
													<div className={alliance.theirSharing[type] ? 'text-green-400' : 'text-zinc-600'}>
														{SHARING_LABELS[type].icon}
													</div>
						...
						{pendingReceived.length > 0 && (
							<div className='space-y-2'>
								...
								{pendingReceived.map((alliance) => (
									<div
										key={alliance._id}
										className='flex items-center justify-between border border-primary/50 bg-primary/20 rounded p-3'
									>
										<div className='flex items-center gap-2'>
											<div
												className='w-4 h-4 rounded border-2 border-zinc-700'
										...
										<div className='flex items-center gap-2'>
											<button
												...
												disabled={loading === alliance._id}
												className='px-2 py-1 text-[10px] font-medium rounded bg-green-600 text-white hover:bg-green-500'
											>
											...
											<button
												...
												disabled={loading === alliance._id}
												className='px-2 py-1 text-[10px] font-medium rounded bg-zinc-800 text-zinc-300 hover:bg-zinc-700'
											>
												Reject
											</button>
										</div>
						...
						{pendingSent.length > 0 && (
							<div className='space-y-2'>
								...
								{pendingSent.map((alliance) => (
									<div
										key={alliance._id}
										className='flex items-center justify-between border border-zinc-800 bg-zinc-900/50 rounded p-3'
									>
										<div className='flex items-center gap-2'>
											<div
												className='w-4 h-4 rounded border-2 border-zinc-700'
					...
					<div className='space-y-2 max-h-80 overflow-y-auto'>
						{availablePlayers.length === 0 ? (
							...
							availablePlayers.map((player) => (
								<div key={player._id} className='flex items-center justify-between border border-zinc-800 bg-zinc-900/50 rounded p-3'>
									<div className='flex items-center gap-2'>
										<div className='w-4 h-4 rounded border-2 border-zinc-700' style={{ backgroundColor: player.color }} />
										<span className='text-sm text-white'>{player.username}</span>
									</div>
									<button
										onClick={() => handleSendInvite(player._id)}
										disabled={loading === player._id}
										className='px-3 py-1 text-xs font-medium rounded bg-blue-600 text-white hover:bg-blue-500 flex items-center gap-1'
									>
										{loading === player._id ? (
											'...'
										) : (
